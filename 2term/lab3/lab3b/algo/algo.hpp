...
namespace ng {
	...
	public:
		...
		template <typename RandomIterator, typename ComparableFunction>
		static RandomIterator partition(RandomIterator left, RandomIterator right, ComparableFunction comparableFunction);

		template <typename T>
		static void quicksort(T* array, const int& left, const int& right, pfunction partition = hmof3partition);

		template <typename RandomIterator, typename ComparableFunction>
		static void quicksort(RandomIterator left, RandomIterator right, ComparableFunction comparableFunction);
	...
	template <typename RandomIterator, typename ComparableFunction>
	RandomIterator algo::partition(RandomIterator left, RandomIterator right, ComparableFunction comparableFunction) {
		...
		while (true) {
			...
			while (comparableFunction(*(++i), *left))
				if (i == right)
					break;

			while (comparableFunction(*left, *(--j)))
				if (j == left)
					break;
	...
	template <typename RandomIterator, typename ComparableFunction>
	void algo::quicksort(RandomIterator left, RandomIterator right, ComparableFunction comparableFunction) {
		...
		RandomIterator plocation = partition(left, right, comparableFunction);

		quicksort(left, plocation - 1, comparableFunction);
		quicksort(plocation + 1, right, comparableFunction);
