...
//#include <cstdio>
//
//const int CNT_FILES = 5;
//const int MAX_CNT_IN_CHUNK = 1000;
//
...
//    for (int i = 0; i < CNT_FILES; ++i)
//        if (i != destination)
//            queue[i] = get_int_from_file(i); // get first number by file index
//
//    int pos = 0;
//
//    while (pos != -1) {
...
//        for (int i = 0; i < CNT_FILES; ++i) {
//
//            // if current file is destination -> countinue
//            if (i == destination)
//                continue;
//
//            // if data in queue == - 1 -> countinue (нет, типа достигли конца одного чанка!!!!!!!!!!!!!!!!!!!!)
//            if (queue[i] == -1)
//                continue;
//
//            if (pos == -1 || queue[pos] > queue[i])
...
//        if (pos != -1) {
//
//            push_int_to_file(destination, queue[pos]); // write to destination selected number
//            queue[pos] = get_int_from_file(pos); // to empty place push new number
//
//        }
//
//    }
...
//        for (int i = 0; i < cnt_in_files.size(); ++i) {
//
//            // if current file (i) not is empty (0 -> amount of chunks)
//            if (!cnt_in_files[i]) {
//
//                // if amount of zero files is more than 1, we are sorted the data !!!
//                if (zero != -1) {
//
//                    sorted = true;
//                    break;
...
//            output_files[zero].open(std::to_string(zero) + "file", std::fstream::binary);
//
//            // go minimal_cnt_chunks amount in for loop
//            for (int i = 0; i < minimal_cnt_chunks; ++i)
//                merge_chunk(zero);
//
//            output_files[zero].close();
//            input_files[zero].open(std::to_string(zero) + "file", std::fstream::binary);
//
//            // update amount of chunks in other files
//            for (int i = 0; i < cnt_in_files.size(); ++i) {
//
//                if (i == zero)
//                    cnt_in_files[i] = minimal_cnt_chunks;
...
//
//        std::fstream infile("../files/" + std::to_string(i) + ".bin", std::ios_base::in | std::ios_base::binary);
//
//        if (!infile.is_open())
