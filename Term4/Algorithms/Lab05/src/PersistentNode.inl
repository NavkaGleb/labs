...
namespace Ng {
    ...
    PersistentNodePtr<Key, Value> PersistentNode<Key, Value>::CloneWithPair(const Pair& pair) const {
        return std::make_shared<PersistentNode>(pair, m_Left, m_Right, m_Color);
    }
    ...
    PersistentNodePtr<Key, Value> PersistentNode<Key, Value>::CloneWithLeft(const SmartPointer& left) const {
        return std::make_shared<PersistentNode>(m_Pair, left, m_Right, m_Color);
    }

    template <typename Key, typename Value>
    PersistentNodePtr<Key, Value> PersistentNode<Key, Value>::CloneWithRight(const SmartPointer& right) const {
        return std::make_shared<PersistentNode>(m_Pair, m_Left, right, m_Color);
    }

    template <typename Key, typename Value>
    PersistentNodePtr<Key, Value> PersistentNode<Key, Value>::CloneAsRed() const {
        return std::make_shared<PersistentNode>(m_Pair, m_Left, m_Right, Color::Red);
    }

    template <typename Key, typename Value>
    PersistentNodePtr<Key, Value> PersistentNode<Key, Value>::CloneAsBlack() const {
        return std::make_shared<PersistentNode>(m_Pair, m_Left, m_Right, Color::Black);
    }

    template <typename Key, typename Value>
    typename PersistentNode<Key, Value>::SmartPointer PersistentNode<Key, Value>::Balance() {
        if (IsBlack()) {
            ...
            if (m_Left && m_Left->IsRed()) {
                ...
                if (m_Left->m_Left && m_Left->m_Left->IsRed()) {
                    ...
                    return std::make_shared<PersistentNode>(m_Left->m_Pair, newLeft, newRight);
                    ...
                    return std::make_shared<PersistentNode>(m_Left->m_Right->m_Pair, newLeft, newRight);
                }
            }
            ...
            if (m_Right && m_Right->IsRed()) {
                if (m_Right->m_Left && m_Right->m_Left->IsRed()) {
                    ...
                    return std::make_shared<PersistentNode>(m_Right->m_Left->m_Pair, newLeft, newRight);
                    ...
                    return std::make_shared<PersistentNode>(m_Right->m_Pair, newLeft, newRight);
                }
            }
        }
    ...
    std::pair<typename PersistentNode<Key, Value>::SmartPointer, bool> PersistentNode<Key, Value>::Push(
        ...
        if (node) {
            ...
            const auto newNode = std::make_shared<PersistentNode>(std::make_pair(key, value));
    ...
    std::pair<typename PersistentNode<Key, Value>::SmartPointer, bool> PersistentNode<Key, Value>::PopLeft(
        ...
        const auto newNode             = std::make_shared<PersistentNode>(
            node->m_Pair,
            newLeft,
            node->m_Right
        );
    ...
    typename PersistentNode<Key, Value>::SmartPointer PersistentNode<Key, Value>::Fuse(
        ...
        if (!left->IsRed() && right->IsRed()) {
            ...
            if (fused && fused->IsRed()) {
                ...
                return std::make_shared<PersistentNode>(fused->m_Pair, newLeft, newRight);
            }
            ...
            return std::make_shared<PersistentNode>(left->m_Pair, left->m_Left, newRight);
    ...
    typename PersistentNode<Key, Value>::SmartPointer PersistentNode<Key, Value>::Balance(const SmartPointer& node) {
        if (node->m_Left && node->m_Left->IsRed() && node->m_Right && node->m_Right->IsRed()) {
            ...
            return std::make_shared<PersistentNode>(node->m_Pair, newLeft, newRight);
        }
    ...
    typename PersistentNode<Key, Value>::SmartPointer PersistentNode<Key, Value>::BalanceLeft(
        ...
        if (node->m_Left && node->m_Left->IsRed()) {
            ...
            return std::make_shared<PersistentNode>(node->m_Pair, newLeft, node->m_Right);
            ...
            const auto newRight = Balance(std::make_shared<PersistentNode>(
                node->m_Right->m_Pair,
            ...
            return std::make_shared<PersistentNode>(node->m_Right->m_Left->m_Pair, newLeft, newRight);
        }
    ...
    typename PersistentNode<Key, Value>::SmartPointer PersistentNode<Key, Value>::BalanceRight(
        ...
        if (node->m_Right && node->m_Right->IsRed()) {
            ...
            return std::make_shared<PersistentNode>(node->m_Pair, node->m_Left, newRight);
            ...
            const auto newLeft = Balance(std::make_shared<PersistentNode>(
                node->m_Left->m_Pair,
            ...
            return std::make_shared<PersistentNode>(node->m_Left->m_Right->m_Pair, newLeft, newRight);
        }
